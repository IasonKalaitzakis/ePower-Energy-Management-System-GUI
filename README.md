...
Figure 3: Example of the real-time values of power and SoC .txt file.
The real-time values of power and SoC are displayed in Fig.3, where:
...
Figure 4: Example of the real-time support signals .txt file.
The real-time ancillary services and their associated values are shown in Fig.4, where:
...
Advanced settings
The advanced settings panel can be toggled by selecting the advanced settings checkbox seen in Fig.5. 
 
Figure 5: Advanced settings panel.
...
Optimization results, real-time values and ancillary services results are sent to the server by the GUI and saved in “User\Documents\ePOWER\GUI_Output” of the host’s PC. The real-time values of the Electric Vehicle are sent to Electric Vehicle Server Provider (EVSP) and to the Electric Vehicle Transaction Operator (EVTO) as seen in the .txt file of Fig.6. The optimization output and real-time grid power exchange values are sent to the power aggregator as seen in Fig. 7 and 8.
 
Figure 6: Example of real-time values of the EV battery.


The real-time values (seen in Fig. 6) transmitted to the aforementioned entities are the following:
Pev: Electric Vehicle power (W)
Vev: Electric Vehicle voltage (V)
Iev: Electric Vehicle current (A)
SoCev: Charge level of Electric Vehicle battery (%).
 
Figure 7: Example of grid power exchange result of optimization.

 
Figure 8: Example of real-time values of the grid side of the inverter.

The .txt file shown in Fig.7 contains the power Pgrid time series output of optimization that the MPC exchanges with the grid. In Fig.8, the real-time values of the grid-side of the MPC are depicted. More specifically:
Vg,a: Grid voltage, phase A (V)
Vg,b: Grid voltage, phase B (V)
Vg,c: Grid voltage, phase C (V)
Ig,a: Grid current, phase A (A)
...
Once optimization is finished, the results are sent to both the real-time controller and the GUI, the latter of which saves them in files. When the “Plot Optimization” button turns on, that means the optimization results have been received, and the user can choose to plot the results. An example of the plots is seen in Fig.9 and the associated files saved are presented in Figs. 10 and 12.
   
Figure 9: Example of optimization result plots.

 
Figure 10: Example of the optimization inputs .txt file.

The .txt file of the optimization input arrays are displayed in Fig.10, where:
...
Figure 11: Example of the optimization results .txt file.
